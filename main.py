...
def login():
    ...
    if returned_message == "Logged In Successfully":
        ...
        return render_template(
            "home.html", returned_data=returned_data, returned_user=returned_user
        )
    else:
        # Flash Message is shown based of the returned message
...
def delete_row_attempt():
    if request.method == "POST":
        ...
        row_to_delete = request.form.get("location")
...
def insert_row_attempt():
    if request.method == "POST":
        ...
        checking_insert = insert_row(
            location_to_insert.strip(),
            comment_to_insert.strip(),
            jobRole_to_insert.strip(),
            company_to_insert.strip(),
        )
